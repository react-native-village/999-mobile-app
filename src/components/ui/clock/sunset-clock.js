...
export default function ClockSunset() {
      ...
      axios
        ...
        .then(locResponse => {
          if (locResponse.data && locResponse.data.results) {
            const currentSolarNoon = locResponse.data.results.solar_noon
            setSolarNoon(currentSolarNoon)
            const SOLAR_TIME = solarNoonStringToMinutes(currentSolarNoon)
            const MIN_TIME = timeStringToMinutes(getCurrentTime())
            let numPolysToColor = computePolysToColor(SOLAR_TIME, MIN_TIME)
            setPolyColors(
              Array(28)
                .fill()
                .map((_, i) =>
  ...
  useEffect(() => {
    // Оставьте это пустым или добавьте другие побочные эффекты, которые вы хотите выполнить при изменении solarNoon или time
  }, [solarNoon])
